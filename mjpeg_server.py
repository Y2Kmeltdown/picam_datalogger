...
def _process_frame(
    ...
    """
    Decode raw XRGB8888 bytes → BGR ndarray, resize to the requested output
    ...
    """
    arr = np.frombuffer(raw, dtype=np.uint8).reshape((src_height, src_width, 4))
    # XRGB8888 on little-endian ARM: channel layout is [B, G, R, X]
    bgr = arr[:, :, :3]
...
async def handle_put_settings(request: web.Request) -> web.Response:
    ...
    Example:
        curl -X PUT http://pi:8080/api/settings \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PiCam MJPEG server — serves scaled camera frames over HTTP "
